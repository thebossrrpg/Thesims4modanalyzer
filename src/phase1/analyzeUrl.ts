...
import type { UnfurlVia, Identity } from "../domain/identity.js";
...
type ProviderKey = "ogWebApi" | "localOgs" | "openGraphXyz" | "iframely" | "microlink";
...
function normalizeUrlForExactMatch(u: string): string {
  ...
  try {
    ...
    const kept = Array.from(parsed.searchParams.entries()).filter(([k, v]) => {
      ...
      if (!key || DROP_KEYS.has(key) || key.startsWith("utm_")) return false;
...
function isSuspiciousTitle(title: string | null): boolean {
 ...
 */
function isBlockedTitle(title: string | null | undefined): boolean {
  if (!title) return true; // sem título = não é útil

  const titleLower = title.toLowerCase().trim();

  if (titleLower.length < 3) return true;
  if (BLOCKED_PATTERNS.test(titleLower)) return true;
 ...
 */
function isUsefulUnfurlMeta(meta: UnfurlMeta | null): boolean {
...
function tryNotionIdentity(url: string): Identity | null {
  ...
  const match = Object.values(snapshotPages as Record<string, { url?: string; filename?: string }>).find((p) => {
  ...
  return {
    ...
    ogSite: "Snapshot",
    isBlocked: false,
    unfurlVia: "notion_direct" as const,
