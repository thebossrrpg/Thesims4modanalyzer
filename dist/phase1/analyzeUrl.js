...
//
// ✅ ALTERAÇÕES PEDIDAS (e só isso):
// - Cadeia de unfurl: OgWebApi → LocalOgs → OpenGraph.io → Iframely (API key) → Microlink
// - tryOpenGraphXyz renomeado logicamente para OpenGraphio
...
function readIframelyUsage() {
    ...
    try {
        ...
        return {
            ...
            cooldownUntilMs: typeof data.cooldownUntilMs === "number" ? data.cooldownUntilMs : undefined,
        };
    }
    catch {
        return { month, hits: 0 };
...
function isSuspiciousTitle(title) {
    ...
    return (/^[A-Z\s!@#$%^&*()]+$/.test(upper) || // tudo caps/símbolos
        title.length < 10 || // curto demais
        !!title.match(/^[A-Z]{3,}$/));
}
...
// ✅ OpenGraph.io (novo na cadeia)
// Observação: o endpoint correto (público) usa app_id.
// Ex.: https://opengraph.io/api/1.1/site/{ENCODED_URL}?app_id=YOUR_APP_ID
async function tryOpenGraphio(url) {
    try {
        const appId = process.env.OPENGRAPHIO_APP_ID ??
            process.env.OPENGRAPH_IO_APP_ID ??
        ...
        const res = await fetch(endpoint, { headers: REQUEST_HEADERS });
        if (!res.ok)
            return null;
        const data = await res.json();
        // Tenta várias “caras” de payload, porque esse serviço muda conforme plano/config
        const g = data?.hybridGraph ??
            data?.openGraph ??
            data?.htmlInferred ??
            data?.html ??
            data ??
            null;
        const title = g?.title ?? g?.ogTitle ?? null;
        const ogSite = g?.site_name ?? g?.ogSiteName ?? g?.publisher ?? null;
        // ⚠️ via: como UnfurlVia não tem "opengraphio" hoje, marcamos como "fallback"
...
// src/phase1/analyzeUrl.ts — tryIframely com API key real
...
// ✅ Microlink (novo no fim da cadeia, depois do Iframely)
// Microlink endpoint configurável via MICROLINK_ENDPOINT (default: https://api.microlink.io/)
// Meta: adiciona `meta=true` para trazer meta tags no payload.
async function tryMicrolink(url) {
    try {
        const endpoint = String(process.env.MICROLINK_ENDPOINT ?? "https://api.microlink.io/").trim() || "https://api.microlink.io/";
        const base = endpoint.endsWith("/") ? endpoint : endpoint + "/";
        const apiKey = String(process.env.MICROLINK_API_KEY ?? "").trim();
        const u = `${base}?url=${encodeURIComponent(url)}&meta=true`;
        const headers = { ...REQUEST_HEADERS };
        if (apiKey) {
            // se você tiver key, manda como header. (Microlink aceita planos com auth; se não tiver, segue sem.)
        ...
        const title = d?.title ??
            ...
            d?.meta?.title ??
            null;
        const ogSite = d?.publisher ??
            d?.meta?.og?.site_name ??
            d?.meta?.og?.siteName ??
            null;
        // ⚠️ via: como UnfurlVia não tem "microlink" hoje, marcamos como "fallback"
        const meta = normalizeMeta({ title, ogTitle: title, ogSite }, "fallback");
        return isUsefulMeta(meta) ? meta : null;
    }
    catch {
...
async function unfurlBlocked(url) {
    ...
    if (cached)
        return cached;
    // Cadeia: ogWebApi → localOgs → OpenGraph.io → iframely → microlink
    if (canTry("ogWebApi")) {
...
export async function analyzeUrl(url) {
    ...
    const isHard404 = titleLower.includes("404") || titleLower.includes("page not found") || titleLower.includes("not found");
